...
subtitle: Out of the box, <a href="https://strapi.io">Strapi.js</a> includes a user-permissions plugin which issues JWT tokens to be stored in client side storage for 'authenticated' requests, this demonstrates how you can modify the plugin's controllers to use server side cookies which allows for httpOnly / secure options.
...
<div className="alert warning">
  If you want to understand why using avoiding use of client side storage is important and a potential security risk, check out these resources for reference:
